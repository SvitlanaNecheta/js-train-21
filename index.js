...
class Musician {
  ...
  #name = String(); // Об'являємо приватні поля #name; #instrument;
  #instrument = String();

  constructor(name, instrument) {
    this.#name = name; // Конструктор приймає два параметри: name та instrument
    this.#instrument = instrument; // присвоєння вхідного значення name до приватного поля #name
    this.count++; // присвоєння вхідного значення instrument до приватного поля #instrument
...
class Bassist extends Musician {
  #band = String(); // Об'являємо приватні поля  #band;
  constructor(name, instrument, band) {
    super(name, instrument);
    this.#band = band;
  } // Конструктор приймає три параметри: name, instrument та band
...
class Band {
  #name;
  #members; // Об'являємо приватні поля #name; #members;
  constructor() {
    this.#name = String();
    this.#members = Array();
  }
  ...
  addMember = (newMember) => {
    ...
    playMusic=>(this.#members).forEach((el)=>super.play);
...
class Performance {
  ...
  get band() {
    return this.#band;}// Створюємо конструктор з трьома вхідними параметрами: #band, #location та #date
  ...
  get location() {
    ...
    get date() {
      ...
      info=()=>{console.log(`Гурт ${this.#band.name} виступить в ${this.#location} ${this.#date.toLocaleDateString()}`)}// Визначаємо метод info(), що виводить рядок в консоль `Гурт ${this.#band.name} виступить в ${this.#location} ${this.#date.toLocaleDateString()}`
}
...
class Concert extends Performance {
  ...
  get ticketPrice(){return this.#ticketPrice}// Створюємо getter для #ticketPrice, що повертає приватну властивість #ticketPrice
  set ticketPrice(newticketPrice){this.#ticketPrice=newticketPrice}// Створюємо setter для #ticketPrice, що дозволяє змінити приватну властивість #ticketPrice
  info=()=>{console.log(`Гурт ${super.band.name} виступить в ${super.location} ${super.date.toLocaleDateString()}, ціна квитка ${this.#ticketPrice}`)}// Визначаємо метод info(), що виводить рядок в консоль `Гурт ${super.band.name} виступить в ${super.location} ${super.date.toLocaleDateString()}, ціна квитка ${this.#ticketPrice}`
}
...
class Vocalist {
  ...
  constructor(name,band){this.#name=name;
  this.#band=band;}// Створюємо конструктор з двома вхідними параметрами: #name та #band
  get name(){return this.#name};// Створюємо getter для #name, що повертає приватну властивість #name
  get band(){return this.#band};// Створюємо getter для #band, що повертає приватну властивість #band
  set name(newName){this.#name=newName};// Створюємо setter для #name, що дозволяє змінити приватну властивість #name
  set band(newBand){this.#band=newBand};// Створюємо setter для #band, що дозволяє змінити приватну властивість #band
  info=()=>{console.log(`Вокаліст ${this.#name} є членом гурту ${this.#band}`)}// Визначаємо метод info(), який виводить інформацію про вокаліста
 ...
 */
class SongWriter {
  #songs=Array();// Об'являємо приватні поля #songs;
  constructor(songs){this.#songs=songs};// Створюємо конструктор з одним вхідним параметром: #songs
  get songs(){return this.#songs;};// Створюємо getter для #songs, що повертає приватну властивість #songs
  addSong=(el)=>{this.#songs.push(el)};// Створюємо метод addSong для додавання нової пісні до масиву #songs
  info=()=>{console.log(`Написав ${this.songs.length} пісень`)}// Визначаємо метод info(), який виводить інформацію про автора пісень
...
band.addMember(guitarist);// Додаємо guitarist до band за допомогою addMember
